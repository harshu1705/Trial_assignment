      ...
      try {
        ...
        context.nodeResults.set(nodeId, {
          ...
          _meta: {
            ...
            endTime,
            duration: 0,
            // Failed nodes can have 0 duration or track it
            inputs: { ...node.data }
            // Best effort inputs
          }
        });
...
var workflowTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      const hasFailure = Array.from(context.nodeResults.values()).some((r) => r.status === "failed");
      const hasSuccess = Array.from(context.nodeResults.values()).some((r) => r.status === "success");
      ...
      const result = {
        ...
        results,
        // ✅ Persist ALL results, including failed ones
      ...
      const failedResult = {
        ...
        nodeStatus,
        results: {}
        // Ensure results is at least empty object
      };
      try {
        await prisma.workflowRun.create({
          data: {
...
@prisma/client/runtime/library.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-HX4WU6F7.mjs.map
